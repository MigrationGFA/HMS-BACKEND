...
| `WARDS.csv` | `WARDS` | Exact `WARD_ID`; `WARD_NAME` → `NAME`; `CODE` = `W{WARD_ID}` |
| `CLINICS.csv` | `CLINICS` | Exact `CLINIC_ID`; unused blank fee/duration columns not stored |
...
Do **not** renumber `USER_ID`, `PERSON_ID`, `ROLE_ID`, `WARD_ID`, `CLINIC_ID`, `DOCTOR_ID`, or `USER_TYPE_ID`. Downstream encounter / admission / prescription CSVs will reference these values.

Table names stay legacy-compatible: `USERS`, `PERSONS`, `ROLES`, `WARDS`, plus new lookups `USER_TYPES`, `CLINICS`, `DOCTORS`.

## Schema prerequisites

Migration `20260813160000_legacy_lookup_tables` adds:

- `USER_TYPES`, `CLINICS`, `DOCTORS`
- Optional `WARDS` columns: `DESCRIPTION`, `HOSPITAL_ID`, `BRANCH_ID`, `DISCONTINUE_FLAG`
...
Recommended order: `user_types` → `roles` → `wards` → `clinics` → `doctors` → `users`, then `persons` in separate commands.
