...
- **Staff booking lifecycle:** Records `/api/records/bookings` lists today’s online bookings. NEW → **Convert** opens Patient Entry with linked `personId` (add demographics, card pay at Cashier, complete + triage). RETURNING → **Check in** requires booking service fee Paid, then creates triage and sets `STATUS=Completed` / `CHECKED_IN_AT`. Cashier `/api/cashier/payments/bookings` collects service fee (NEW excludes reg/card already on card queue).
- **Public catalog:** `GET /api/billing/services/bookable` returns mode, duration, and `generalPrice` for landing.
- **Frontend:** Superadmin Service Billing wires create → price → approve (mode/duration/capacity on create or Capacity dialog); landing `/appointment` wizard; Records `/hms/identity` Online Booking channel; Cashier `/dashboard/cashier/bookings`.
