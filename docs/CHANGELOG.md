...
### Fixed
- **Records draft resume after payment:** `GET /api/records/registrations/resume` resolves person/card, `paymentCleared`, and `suggestedStep` when reopening IndexedDB drafts. Drafts (Dexie v2) persist `registeredPersonId` after Create Record & Card; **Continue Draft** syncs payment from Cashier instead of offering duplicate create (409). Step 4 auto-refreshes payment status when `personId` is known. **Deploy:** backend redeploy only (no migration).
- **Records Patient Entry wizard:** Migration `20260805180000_registration_charge_services` upserts ACTIVE Master Services `SVC-REG-FEE`, `SVC-CARD-FEE`, `SVC-REG-CONSULT` so `GET /api/records/registration-charges` works after `migrate deploy` without seed. fnph-aro Patient Entry Engine moves registration charges + **Create Record & Card** to step 4 **Payment** (step 3 NOK only); removes cashier payment-type fields from Records; Medical & Social optional with **Not available** coercion on complete. **Deploy:** `npx prisma migrate deploy`.
...
### Added
- **Cashier Bills invoice tab:** Invoice tab on `/dashboard/cashier/bills` uses live patient search (`GET /api/cashier/patients/recent|search`) and per-patient outstanding bills instead of IndexedDB seed invoices.
- **Lab Request Center blank page:** Fixed missing `UserPlus`, `QrCode`, `FilePlus` icon imports in `LabRequestCenterPage.tsx`. Seed ACTIVE Master Services `SVC-REG-FEE`, `SVC-CARD-FEE`, `SVC-REG-CONSULT` with GENERAL prices. `GET /api/records/registration-charges` (`card:read`) returns reg/card/consult bundle via `ServiceCatalogService.resolveRegistrationCharges`. `POST /api/records/registrations` ignores client fee fields and applies catalog-resolved amounts. fnph-aro Patient Entry Engine loads read-only fees on Payment step (step 4) with loading/retry states; see migration `20260805180000_registration_charge_services` for deploy without seed.
...
- **Public bookable catalog:** `GET /api/billing/services/bookable` (no auth) returns ACTIVE + `ONLINE_BOOKABLE` services for the landing `/appointment` Service step. Wired in fnph-aro `AppointmentPage` + Superadmin Service Billing UI (`ServiceBillingLive`) against Master Service Catalog APIs.
- **Master Service Catalog:** Unified hospital services & pricing in HMS-BACKEND. Prisma models `Departments`, `ServiceCategories`, `MasterServices`, `ServicePayers`, `ServicePayerPrices`, `ServicePriceApprovals` (migration `20260731120000_master_service_catalog`); nullable `SERVICE_ID` FK on `LAB_TESTS` / `IMAGING_STUDIES` / `ADMISSION_BILLING_ITEMS`. Workflow: department `POST /api/billing/services` (no prices) → finance `PATCH …/pricing` → IT `POST …/approve|reject`. Prices: `GENERAL_PRICE` + `STAFF_PRICE` on service; NHIA/HMO/Corporate via `ServicePayerPrices`. APIs under `/api/billing/service-categories|departments|services|service-payers`. RBAC `service:read|create|update|price|approve`, `service_payer:manage`. Lab/imaging order snapshots use master `GENERAL_PRICE` when linked; domain `UNIT_PRICE` kept as synced cache. Seed upserts categories, departments, NHIA payer, FE-mock consult/therapy/ward services + backfill. **Deploy:** `npx prisma migrate deploy`; optional `npm run prisma:seed`.
- **Doctor CDS / Analytics / Cross-Dept live:** CDS page wires `clinical-pharmacy` alerts/check/allergies (pregnancy/renal/liver/guidelines = Pending Integration). Analytics confirmed on `GET /api/doctor/analytics` with honest CSV (Excel-compatible) exports + PendingIntegrationBanner offline. Cross-Dept aggregates referrals + critical lab/imaging + notifications for pending/tracker/critical/completed tabs; Module Status / Messages / Sync = Pending Integration. FE compositor `doctorCrossDept.ts`. Seeds stripped when `VITE_USE_API` on.
- **Doctor clinical boards live (Admission / Diagnosis / Results / Emergency):** Admission Admitted tab uses `GET /api/admissions` (no Dexie/KPI padding); History from admission-requests + admissions; Diagnosis Coding Support from catalog (no hardcoded GMPC/OPC lists); Results Imaging via `listImagingReports` + critical ack APIs + lab history; ECG / PACS / Export / Message / psych scales show visible **Pending Integration** banner. Emergency override create-actions (new admission/referral/prescribe/export/message) marked Pending Integration. Shared FE component `PendingIntegrationBanner`.
- **Doctor clinical tabs live (Prescriptions / Lab / Imaging):** Medications APIs `GET /api/prescriptions/medications`, `POST …/:id/items/:itemId/stop`, `POST …/:id/refill`, `POST/GET /api/prescriptions/external`. Lab critical ack: `CRITICAL_*` on `LAB_RESULTS` (migration `20260727210000_lab_result_critical_ack`); `GET /api/laboratory/results?critical=&personId=`; `POST …/results/:id/acknowledge`. Imaging reports: `IMAGING_REPORTS` (migration `20260727200000_imaging_reports`); `GET/POST /api/radiology/imaging/reports`, `POST …/reports/:id/acknowledge`. Frontend doctor pages strip seed/KPI padding when `VITE_USE_API` on; lab drafts via IndexedDB. **Deploy:** `npx prisma migrate deploy`.
- **Cashier reports / verify / audit / settings:** Migration `20260727190000_cashier_reports_settings` adds singleton `CASHIER_SETTINGS`. APIs: `GET /api/cashier/reports`, `GET /api/cashier/receipts/verify`, `POST /api/cashier/receipts/:id/reprint`, `GET /api/cashier/audit` (+ `/stats`), `GET/PATCH /api/cashier/settings`. RBAC `cashier:report-read`, `cashier:settings-read|update`. Frontend pages `/dashboard/cashier/reports|verify|audit|settings` wired live; My Profile at `/dashboard/cashier/profile` via `/users/me`. **Deploy:** `npx prisma migrate deploy`.
- **Cashier ops (receipts / refunds / discounts / shifts):** Migration `20260727180000_cashier_ops` adds `CASHIER_PAYMENT_RECEIPTS`, `CASHIER_REFUND_REQUESTS`, `CASHIER_DISCOUNT_REQUESTS`, `CASHIER_SHIFTS`. Payment confirms write idempotent receipts. APIs under `/api/cashier/receipts|refunds|discounts|shifts` with RBAC `cashier:receipt-read`, `cashier:refund-request|approve`, `cashier:discount-request|approve`, `cashier:shift-open|close|read|approve`. Frontend Refunds (partial/full amount), Discounts (eligible unpaid bills), Daily Closing wired live; Bills hub includes imaging. **Deploy:** `npx prisma migrate deploy`.
- **Cashier Bills & Payments hub:** Single page `/dashboard/cashier/bills` with summary cards and tabs (Invoice | Pending | Paid | Part). Old routes `/pay`, `/pending`, `/paid`, `/part` redirect to `?tab=`. Nav collapses four items into one.
- **Cashier page dedupe:** Canonical desk remains `/dashboard/cashier`. Removed dead HmsModules `BillingPos` / `CashierOverview`. Superadmin mock `/dashboard/superadmin/billing/cashier` redirects to live cashier; Lab/Nurse “Open Cashier” deep-links to `/dashboard/cashier/pharmacy`. HMS `sidebars.cashier` now uses `cashierNav`. Billing Initiation `/billing/*` (visits/gate/POS) kept.
- **Records Officer Overview live:** Frontend `/dashboard/records` uses `GET /api/records/overview` (KPIs, pending tasks, alerts, recent activity, today’s arrivals). Old HmsModules partial dashboard-stats view removed; empty states when no data (no client mock activity).
- **Records My Profile:** Frontend `/records/profile` for Health Records officers via existing `GET/PATCH /api/users/me` + `POST /api/auth/change-password` (no new permissions). UI edits identity (`firstName`/`lastName`), phone, department, desk/unit (`clinicName`), station/window (`wardAssignment`), qualifications, and duty hours (`consultationHours`). Omits clinical license/specialty fields. Prefs stored client-side (`fnph_records_prefs_v1`).
- **Records Retrieval / Archive / Reports / Analytics:** Migration `20260727160000_records_file_archive_reports` adds `RECORD_FILE_REQUESTS` (+ events), `RECORD_ARCHIVES`, `RECORD_REPORT_SNAPSHOTS`. APIs under `/api/records/file-requests`, `/archives`, `/reports/*`, `/analytics` with RBAC `records-file:*`, `records-archive:*`, `records-report:*`, `records-analytics:read`. Frontend pages `/records/retrieval|archive|reports|analytics` wired live. **Deploy:** `npx prisma migrate deploy`.
- **Clinical Pharmacy (interactions):** Migration `20260727140000_clinical_pharmacy` adds `PATIENT_ALLERGIES`, `DRUG_INTERACTION_RULES`, `CLINICAL_PHARMACY_ALERTS` (+ seed rules). APIs `/api/clinical-pharmacy/*` (alerts worklist/KPIs, check, override, notify, close, rules CRUD, allergies). RBAC `clinical-pharmacy:read|update|manage-rules`. Frontend `/pharmacy/interactions` live worklist with Check / Override / Notify dialogs. **Deploy:** `npx prisma migrate deploy`.
- **Pharmacy My Profile + live Support requests:** Frontend `/pharmacy/profile` and `/dashboard/pharmacy/profile` via existing `GET/PATCH /users/me` + `POST /auth/change-password`. Prisma `SUPPORT_REQUESTS` (migration `20260727120000_support_requests`); APIs `POST/GET/PATCH /api/support-requests` with RBAC `support:create|read|update` (HR role gains update + list-all). Shared header `SupportButton` posts live tickets; HR `/dashboard/hr/support` lists and updates status. **Deploy:** `npx prisma migrate deploy`.
- **Laboratory Histopathology / QC / Config / Profile:** Migration `20260724190000_lab_histo_qc` adds `LAB_HISTOPATHOLOGY_CASES`, `LAB_QC_RUNS`, and seeds catalog gaps (`MP`, `XRC`). APIs: `/api/laboratory/histopathology/*`, `/qc/*`. Lab Config FE is live `LAB_TESTS` admin (`GET/POST/PATCH /tests`). Lab My Profile at `/dashboard/laboratory/profile` via `GET/PATCH /users/me`. Mocks removed; RequireRole on histo/qc/config/profile. **Deploy:** `npx prisma migrate deploy`.
- **Laboratory SFA / Analytics / Specimens / Microbiology:** Migration `20260724180000_lab_sfa_specimens` adds `LAB_SFA_ANALYSES`, `LAB_SPECIMEN_TRACKING` / `LAB_SPECIMEN_EVENTS`. APIs: `/api/laboratory/sfa/*`, `GET /analytics/summary`, `/specimens/*`, `/microbiology/*` (cultures wrapper). Drug-screen list returns KPIs; FE worklist board with Draft/In progress/Completed tabs. Live SFA, Analytics, Specimen Tracking, Microbiology pages (mocks removed). **Deploy:** `npx prisma migrate deploy`.
- **Laboratory specialty + Blood Bank hardening:** Migration `20260724160000_lab_specialty_blood_donors` adds `LAB_DRUG_SCREENS` / `LAB_DRUG_SCREEN_RESULTS`, `LAB_CULTURES` / `LAB_CULTURE_SENSITIVITIES`, `LAB_REPORT_SNAPSHOTS`, and `BLOOD_DONORS` (+ unit/request `DONOR_ID` / `DOCTOR_ID`). APIs: `/api/laboratory/drug-screens/*`, `/cultures/*`, `/reports` (+ `POST generate`), blood-bank `/donors`, `/doctors?q=`, `/issue-history`; multi-unit FEFO issue; Issued→Available blocked. Frontend: live Drug Screen / Culture / Reports / Blood Bank (donors, searchable doctor, issue unit picker, history); mocks removed. **Deploy:** `npx prisma migrate deploy`.
- **Laboratory Dashboard Overview:** `GET /api/laboratory/overview` returns today’s LIS KPIs (requests, unpaid, awaiting collection, samples collected, awaiting validation, released, STAT emergency, revenue), recent activity, STAT/Urgent alerts, and pending task counts. Permission `lab:read`. Frontend `/dashboard/laboratory` (`LabCenterDashboard`) loads live data with loading/error/empty states (no IndexedDB mock KPIs or hardcoded alerts).
- **Doctor Clinical Workstation Overview:** `GET /api/doctor/overview` returns today’s operational KPIs (waiting queue, active consults, pending lab/imaging, admission requests, ward inpatients, referrals, discharge drafts, emergency triage) plus a queue preview and tab hints. Permission `encounter:read`. Frontend `/dashboard/doctor/clinical` (`DocOverview`) loads live data with loading/error/empty states (no hardcoded mock KPIs).
- **Doctor Emergency Override + Research & Audit APIs:** Prisma `EMERGENCY_OVERRIDE_SESSIONS` / `EMERGENCY_CRITICAL_ALERTS` (migration `20260724120000_emergency_override`) and `RESEARCH_REGISTRY_ENTRIES` / `RESEARCH_TRIALS` / `CLINICAL_AUDIT_PROJECTS` (migration `20260724130000_doctor_research`). APIs `/api/emergency-override/*` (board, sessions, alerts, admissions, referrals, medications) with RBAC `emergency-override:read|create|update`; `/api/doctor/research/*` (summary, diagnoses, admissions-by-ward, drug-utilization, registry/trials/audit-projects CRUD) with `doctor-analytics:read` + `doctor-research:write`. `GET /api/audit/stats` for Audit & Compliance KPIs. Frontend doctor boards strip seed/localStorage mocks and show **0** when empty. **Deploy:** `npx prisma migrate deploy`.
- **Laboratory Dashboard Integration:** Unified full `labCenterNav` on all lab pages (fixes disappearing sidebar). Walk-In hardened (errors, catalog reload after patient, paid queue error state, Cashier/Samples deep-links). Templates **Preview** renders sample result layout from backend `FIELDS` only. Blood bank domain: `BLOOD_UNITS` / `BLOOD_REQUESTS` / `BLOOD_REQUEST_EVENTS` / `BLOOD_CROSSMATCHES` (migration `20260722140000_lab_blood_bank`), RBAC `blood-bank:read|create|update|issue` (Lab + Admin/CMD), APIs under `/api/laboratory/blood-bank/*`, seed units + pending requests. Patient history `GET /api/laboratory/history?personId=`. Frontend Blood Bank + History Centers live (mocks removed). **Deploy:** `npx prisma migrate deploy`; optional seed.
- **Doctor Clinical Surfaces:** Coming-soon stubs for Schedule / Appointments / Tasks (zero KPIs). Live Certificates & Reports (`/api/clinical-certificates` + jspdf download), Doctor Analytics (`GET /api/doctor/analytics`), and My Profile (`GET/PATCH /api/users/me`, `POST /api/auth/change-password`). RBAC `certificate:*`, `doctor-analytics:read`. USERS profile columns + certificate tables (migration `20260722120000_doctor_profile_and_certificates`). Seed 16 certificate templates. **Deploy:** `npx prisma migrate deploy`; optional `npm run prisma:seed`.
- **Doctor Clinical Boards + Discharge drafts:** Prisma `DISCHARGE_DRAFTS` / `DISCHARGE_DRAFT_EVENTS` (migration `20260721200000_discharge_drafts`). APIs `/api/discharge-drafts/*` (create/list/update/submit/payment-status/clear-payment/return/finalize/cancel). RBAC `discharge:create|read|update|clear-payment|finalize`. Submit calls `order-discharge`; finalize requires `PaymentCleared` then `completeDischarge` (bed `CLEANING`). `PATCH /admissions/:id/complete-discharge` gated to `discharge:finalize`. Empty `/api/discharge` returns 403. Payment aggregate covers unpaid admission/lab/imaging/Rx/pharmacy walk-in. Frontend: Doctor discharge + Patient Directory + Active Board + Ward Round (Ward Round Note via clinical-notes); Cashier `?tab=discharge`; Records `/records/discharge`. Seed demo drafts. **Deploy:** `npx prisma migrate deploy`.
...
### Changed
- **Prod start always migrates:** `npm run start:prod` now runs `npx prisma migrate deploy` before `node dist/...` so new tables (e.g. `EMERGENCY_OVERRIDE_SESSIONS`) are created even if the Render dashboard start command was only `npm run start:prod`. Fixes production 500s on `/api/emergency-override/board|sessions|alerts` when migrations lagged the API code.
