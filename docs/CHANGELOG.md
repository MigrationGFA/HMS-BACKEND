...
### Fixed
- **Staff chat delivery:** Inbox lists all conversations where the user is a participant (removed `moduleScope` filter that hid cross-module DMs). New conversations compute `moduleScope` as the union of all participants' modules. WebSocket payloads use per-recipient unread counts. Frontend dedupes messages on send, keeps a persistent `/chat` socket via `ChatNotificationProvider`, shows toast + header badge for unread messages when off the comms screen, and supports `?thread=` deep links.
